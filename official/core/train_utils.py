...
def create_optimizer(task: base_task.Task,
  ...
  if 'dp_config' in inspect.signature(task.create_optimizer).parameters:
    ...
    return task.create_optimizer(
        params.trainer.optimizer_config, params.runtime, dp_config=dp_config)
  else:
    if hasattr(params.task, 'differential_privacy_config'
    ...
    return task.create_optimizer(params.trainer.optimizer_config,
                                 params.runtime)
