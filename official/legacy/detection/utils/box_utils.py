...
def yxyx_to_xywh(boxes):
  ...
  return np.stack([boxes_xmin, boxes_ymin, boxes_width, boxes_height], axis=-1)
...
def compute_outer_boxes(boxes, image_shape, scale=1.0):
  ...
  if scale < 1.0:
    raise ValueError(
        f'scale is {scale}, but outer box scale must be greater than 1.0.')
...
def encode_boxes(boxes, anchors, weights=None):
  ...
  with tf.name_scope('encode_boxes'):
    ...
    return tf.concat([encoded_dy, encoded_dx, encoded_dh, encoded_dw], axis=-1)
